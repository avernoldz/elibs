...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header("Location: admin_thesiscatalog.php?status=invalid");
    exit();
}
...
if (!$thesis) {
    header("Location: admin_thesiscatalog.php?status=notfound");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $image_path = $thesis['abstract_image'];
    if (!empty($_FILES['abstract_image']['name'])) {
        ...
        if ($_FILES["abstract_image"]["size"] > 5 * 1024 * 1024) {
            header("Location: admin_editthesis.php?id=$thesis_id&status=largefile");
            exit();
        } else {
            if (move_uploaded_file($_FILES["abstract_image"]["tmp_name"], $target_file)) {
                $image_path = $target_file;
            } else {
                header("Location: admin_editthesis.php?id=$thesis_id&status=uploadfail");
                exit();
            }
        }
    }
    ...
    if ($stmt->execute()) {
        header("Location: admin_editthesis.php?id=$thesis_id&status=success");
    } else {
        header("Location: admin_editthesis.php?id=$thesis_id&status=error");
    }
    $stmt->close();
    exit();
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (status === 'success') {
        ...
        Swal.fire({
            title: 'Error!',
            text: 'There was an issue updating the thesis.',
            icon: 'error',
            confirmButtonText: 'OK'
        });
